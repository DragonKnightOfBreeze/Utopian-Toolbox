...
class YamlLanguageSchemaBasedFoldingBuilder : FoldingBuilder, DumbAware {
    ...
    private fun collectDescriptorsRecursively(node: ASTNode, document: Document, descriptors: MutableList<FoldingDescriptor>) {
        run {
            ...
            if(element !is YAMLSequenceItem) return@run
            val jElement = element.value?.toJElementOfType<JObject>() ?: return@run
